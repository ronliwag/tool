...
class OnlineFeatureExtractor:
    ...
    def __call__(self, new_samples, sr=ORG_SAMPLE_RATE):
        samples = new_samples
        print(f"DEBUG: samples type: {type(samples)}, shape: {samples.shape if hasattr(samples, 'shape') else 'no shape'}, length: {len(samples)}")
        ...
        if effective_num_samples <= 0 or len(samples) < effective_num_samples:
            print(f"DEBUG: Not enough samples. effective_num_samples: {effective_num_samples}, len(samples): {len(samples)}")
            # Return empty tensor if not enough samples
            return torch.tensor([], device=self.device)
            
        samples = samples[:effective_num_samples]
        print(f"DEBUG: Using {len(samples)} samples for feature extraction")
        # Convert 1D array to 2D tensor (channels x length) as expected by convert_waveform
        if isinstance(samples, torch.Tensor):
            # If samples is already a tensor, reshape it to 2D
            if samples.dim() == 1:
                waveform_2d = samples.unsqueeze(0)  # Add channel dimension
            else:
                waveform_2d = samples
        else:
            # If samples is a numpy array, convert to 2D tensor
            waveform_2d = torch.tensor([samples], dtype=torch.float32)
        waveform, sample_rate = convert_waveform(
            waveform_2d, sr, to_mono=True, to_sample_rate=16000
        )
...
class StreamSpeechS2STAgent(SpeechToSpeechAgent):
    ...
    def policy(self):
        ...
        for i, hypo in enumerate(finalized_st):
            ...
            global ST
            ST[len(self.states.source)]=text
        ...
        if isinstance(new_wav, torch.Tensor):
            new_wav_np = new_wav.detach().cpu().numpy()
        else:
            new_wav_np = new_wav
        
        S2ST.extend(new_wav_np.tolist())
        global OFFSET_MS
        if OFFSET_MS==-1:
...
def run(source):
    ...
    while not agent.states.target_finished:
        cur_idx+=interval
        # Convert numpy array to tensor properly to avoid the slow conversion warning
        source_data = samples[:cur_idx]
        if isinstance(source_data, np.ndarray):
            source_data = torch.from_numpy(source_data).float()
        agent.states.source = source_data
...
def uploaded_file(filename):
    try:
        ...
        # pdb.set_trace()
        # if len(S2ST)==0:
        print("Resetting and running...")
        reset()
        run(path)
        print(f"S2ST length: {len(S2ST)}")
        
        output_path = os.path.join(os.path.dirname(path), 'output.' + os.path.basename(path))
        # Convert S2ST list to numpy array for efficient writing
        ...
        input_wav_path = os.path.join(os.path.dirname(path), 'input.' + os.path.basename(path))
        left.export(input_wav_path, format="wav")
        right.export(output_path, format="wav")
        print(f"Input written to: {input_wav_path}")
