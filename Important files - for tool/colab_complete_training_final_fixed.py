#!/usr/bin/env python3
"""
FINAL FIXED TRAINING SCRIPT - COMPLETELY WORKING
Fixes metadata structure mismatch and all other issues
"""
...
from odconv_simple import ODConvTranspose1D
from grc_lora_working import MultiReceptiveFieldFusion
...
class ModifiedHiFiGANGenerator(nn.Module):
    ...
    def __init__(self, speaker_dim=192, emotion_dim=256):
        ...
        # MRF layers with working implementation
        self.mrfs = nn.ModuleList([
            MultiReceptiveFieldFusion(512),
            MultiReceptiveFieldFusion(256),
            MultiReceptiveFieldFusion(128),
            MultiReceptiveFieldFusion(64),
            MultiReceptiveFieldFusion(32),
        ])
    ...
    def forward(self, mel, speaker_embed, emotion_embed):
        ...
        # Set condition for all ODConv layers
        for up in self.ups:
...
class RealCVSSDataset(torch.utils.data.Dataset):
    """FIXED CVSS Dataset with correct metadata handling"""
    ...
    def __init__(self, config, speaker_extractor, emotion_extractor):
        ...
        print(f"Raw metadata type: {type(self.metadata)}")
        print(f"Raw metadata length: {len(self.metadata) if isinstance(self.metadata, list) else 'Not a list'}")
        ...
        if isinstance(self.metadata, list):
            ...
            raise ValueError(f"Unknown metadata format: {type(self.metadata)}")
        
        print(f"Processed pairs: {len(pairs)}")
        
        # Pre-compute embeddings with FIXED file path handling
        print("Pre-computing embeddings for faster training...")
        self.embeddings_cache = {}
        successful_count = 0
        failed_count = 0
        
        for i, pair in enumerate(tqdm(pairs, desc="Processing embeddings")):
            try:
                # FIXED: Handle different metadata key formats
                spanish_path = None
                english_path = None
                
                # Try different possible key names for Spanish audio
                if 'spanish_audio_path' in pair:
                    spanish_path = pair['spanish_audio_path']
                elif 'spanish' in pair:
                    spanish_path = os.path.join(config.dataset_path, pair['spanish'])
                elif 'spanish_audio' in pair:
                    spanish_path = os.path.join(config.dataset_path, 'spanish', pair['spanish_audio'])
                
                # Try different possible key names for English audio
                if 'english_audio_path' in pair:
                    english_path = pair['english_audio_path']
                elif 'english' in pair:
                    english_path = os.path.join(config.dataset_path, pair['english'])
                elif 'english_audio' in pair:
                    english_path = os.path.join(config.dataset_path, 'english', pair['english_audio'])
                
                if not spanish_path or not english_path:
                    print(f"ERROR: Missing audio paths for pair {i}")
                ...
                if not os.path.exists(spanish_path):
                    print(f"ERROR: Spanish file not found: {spanish_path}")
                    failed_count += 1
                    continue
                
                if not os.path.exists(english_path):
                    print(f"ERROR: English file not found: {english_path}")
                    failed_count += 1
                    continue
                ...
                # Validate embeddings
                if speaker_embed is None or len(speaker_embed) == 0:
                    print(f"ERROR: Invalid speaker embedding for pair {i}")
                    failed_count += 1
                    continue
                
                if emotion_embed is None or len(emotion_embed) == 0:
                    print(f"ERROR: Invalid emotion embedding for pair {i}")
                    failed_count += 1
                    continue
                
                # Store successful pair
                ...
                print(f"ERROR: Failed to process pair {i}: {e}")
                failed_count += 1
                continue
        ...
        if successful_count == 0:
            raise ValueError("No valid samples found! Check your dataset structure.")
    ...
    def __getitem__(self, idx):
        ...
        spanish_path = embeddings_data['spanish_path']
        english_path = embeddings_data['english_path']
...
class ProfessionalTrainer:
    ...
    def train_epoch(self, dataloader):
        ...
        for batch_idx, (mel, audio, speaker_embed, emotion_embed) in enumerate(dataloader):
            ...
            # Calculate loss
            loss = self.l1_loss(generated_audio, audio.unsqueeze(1))
