...
class WorkingODConvIntegration:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.spk_extractor = RealECAPAExtractor()
        except Exception as _e:
            print(f"[REAL ODConv] WARN: Speaker extractor unavailable: {_e}")
        try:
            from real_emotion_extractor import RealEmotionExtractor
            self.emo_extractor = RealEmotionExtractor()
        except Exception as _e:
            print(f"[REAL ODConv] WARN: Emotion extractor unavailable: {_e}")
    ...
    def _initialize_generator(self):
        ...
        try:
            # Import ODConv and GRC+LoRA (real ODConv implementation)
            from odconv_real import ODConvTranspose1D
    ...
    def process_audio_with_odconv(self, audio_path=None, mel_features=None, audio_tensor=None):
        ...
        try:
            ...
            # Prepare REAL conditioning embeddings (no randoms)
            speaker_embed = None
            emotion_embed = None
            try:
                if audio_path is not None and self.spk_extractor is not None:
                    se = self.spk_extractor.get_speaker_embedding(audio_path)
                    if isinstance(se, torch.Tensor):
                        speaker_embed = se.float().unsqueeze(0).to(self.device)  # [1, 192]
                if audio_path is not None and self.emo_extractor is not None:
                    ee = self.emo_extractor.get_emotion_embedding(audio_path)
                    if isinstance(ee, torch.Tensor):
                        emotion_embed = ee.float().unsqueeze(0).to(self.device)  # [1, 256]
            except Exception as _emb_err:
                print(f"[REAL ODConv] WARN: Embedding extraction issue: {_emb_err}")

            # If extractors not available, use safe zero embeddings (no dummies/randoms)
            if speaker_embed is None:
                speaker_embed = torch.zeros(1, self.config['training_config']['speaker_dim'], device=self.device)
            if emotion_embed is None:
            ...
            try:
                if hasattr(self.generator.ups[0], 'last_alphas') and self.generator.ups[0].last_alphas is not None:
                    la = self.generator.ups[0].last_alphas.numpy().tolist()
                    print(f"[REAL ODConv] Gating (ups[0]) mean alphas: {la}")
            except Exception:
                pass
