...
class GroupedResidualConvolution(nn.Module):
    ...
    def __init__(self, channels, kernel_size=3, dilation=1, groups=1):
        ...
        # LoRA adapters (Conv1d low-rank update): delta = A(ReLU(B(x)))
        lora_rank = max(1, min(8, channels // 4))
        self.lora_down = nn.Conv1d(channels, lora_rank, kernel_size=1, bias=False)
        self.lora_up = nn.Conv1d(lora_rank, channels, kernel_size=1, bias=False)
        self.lora_act = nn.LeakyReLU(0.2)
    ...
    def forward(self, x):
        ...
        # LoRA delta computed on the residual path (lightweight)
        try:
            lora_delta = self.lora_up(self.lora_act(self.lora_down(residual)))
        except Exception:
            lora_delta = 0.0
